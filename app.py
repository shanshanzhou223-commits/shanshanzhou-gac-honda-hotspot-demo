...
def render_content_playbook(topic: dict, vehicle_key: str, title: str = "内容演绎方案"):
    ...
    with st.expander("🎬 查看视频分镜脚本"):
        scripts = playbook["视频脚本"]
        # 按时长分组：每个时长的第一行是元信息，后面是分镜
        duration_groups = {}
        ...
        for row in scripts:
            ...
            if row["镜号"] == "—":
                ...
                duration_groups[current_duration] = {
                    "内容角度": row["内容角度"],
                    "适合场景": row["适合场景"],
                    "结构说明": row["结构说明"],
                    "shots": [],
                ...
                if current_duration:
                    duration_groups[current_duration]["shots"].append(row)

        duration_tabs = st.tabs(list(duration_groups.keys()))
        for dur, dtab in zip(duration_groups.keys(), duration_tabs):
            info = duration_groups[dur]
            with dtab:
                st.markdown(f"**📌 内容角度**：{info['内容角度']}")
                st.caption(f"**适合场景**：{info['适合场景']}｜**三段式结构**：{info['结构说明']}")
                ...
                st.dataframe(
                    ...
                    hide_index=True,
                    column_order=["环节", "镜号", "时长", "画面描述", "台词/字幕", "音效/音乐"],
                )
