...
def render_content_playbook(topic: dict, vehicle_key: str, title: str = "内容演绎方案"):
    ...
    with st.expander("🎬 查看视频分镜脚本"):
        ...
        # 兼容处理：
        # 1. 新版：scripts 是 List[Dict]，每个 Dict 有 "acts" 和 "shots"
        # 2. 旧版/缓存版：scripts 是平铺 List[Dict]，每条是一个 shot，用 "画面" 等字段
        is_structured = (
        ...
        if is_structured:
            duration_tabs = st.tabs([d["时长"] for d in scripts])
            for d, dtab in zip(scripts, duration_tabs):
                with dtab:
                    st.markdown(f"**📌 内容角度**：{d['内容角度']}")
                    st.caption(
                        f"**适合场景**：{d['适合场景']}｜**三段式结构**：{d['结构说明']}"
                    )
                    shots = []
                    for act in d["acts"]:
        ...
        elif isinstance(scripts, list) and len(scripts) > 0 and "画面" in scripts[0]:
            # 兜底：旧版平铺格式直接展示
